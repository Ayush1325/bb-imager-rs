...
/// The lone customization offered by the BCF and Zepto flashers, which differ
/// only in the [`FlashingCustomization`] variant they round-trip through.
fn verify_toggle<'a>(
    config: &'a persistance::BcfCustomization,
    wrap: impl Fn(persistance::BcfCustomization) -> FlashingCustomization + 'a,
) -> Element<'a, BBImagerMessage> {
    widget::container(
        widget::toggler(!config.verify)
            .label("Skip Verification")
            .on_toggle(move |x| {
                BBImagerMessage::UpdateFlashConfig(wrap(config.clone().update_verify(!x)))
            }),
    )
